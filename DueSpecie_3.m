...
s1.density = 1.5 * max((exp(-((eco.X-700).^2 + (eco.Y-700).^2)/(4000)^2) - eps), 0);
...
video = VideoWriter('Aquila_2.avi');
video.FrameRate = 30;
open(video)
...
while answer == "Yes"
    ...
    for i = 1:1000
        ...
        title(sprintf("%.0f years s1/s2=%.2f", eco.t, sum(sum(s1.density)) / sum(sum(s2.density))));
